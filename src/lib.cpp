...
int manipulation_filters::filter_lowpass_pre(string path, string pathSave)
{
...
int manipulation_filters::filter_lowpass(string path, string frequency_input, string gain_input, string suppress_high_freq_l, string suppress_high_freq_r, string pathSave)
{
...
int manipulation_filters::filter_highpass_pre(string path, string pathSave)
{
...
int manipulation_filters::filter_highpass(string path, string frequency_input, string gain_input, string suppress_low_freq_l, string suppress_low_freq_r, string pathSave)
{
  string frequency = frequency_input; //Hz, low pass
  string gain_     = gain_input; //~db
  command  *obj_exec      = new command();
...
int manipulation_filters::filter_bandpass(string path, string frequency_l_input, string frequency_r_input, string gain_l_input, string gain_r_input, string q_in_left, string q_in_right, string pathSave)
{
  ...
  string pSave      = (pathSave.c_str() != NULL)? pathSave : "arquivoProcessado.wav";
  command  *obj_exec      = new command();
  string c = "pd  -open \"" + loc_patch +"\" -send \"local "+ path +"\" -send \"pd dsp 1\" -send \"controlBnpl 1\" -send \"controlBnpr 1\" -send \"qbnpl " + q_l + "\" -send \"qbnpr " + q_r + "\" -send \"fbnpr " + frequency_l + "\" -send \"fbnpl " + frequency_r + "\" -send \"ganhoBnpl " + gain_l + "\" -send \"ganhoBnpr " + gain_r + "\" -send \"save "+ pSave  +"\"" ;
  //cout << c << endl;
  return obj_exec -> do_command(c);
}
